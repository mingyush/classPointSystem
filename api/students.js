...
router.get('/search', authenticateToken, requireTeacher,
    asyncHandler(async (req, res) => {
        ...
        if (!keyword || typeof keyword !== 'string' || keyword.trim().length === 0) {
            throw createError('VALIDATION_ERROR', '搜索关键词不能为空');
        }
...
router.get('/:id', authenticateToken, 
    asyncHandler(async (req, res) => {
        ...
        if (req.user.userType === 'student' && req.user.userId !== id) {
            throw createError('PERMISSION_DENIED', '学生只能查看自己的信息');
        }
...
router.get('/:id/rank', authenticateToken,
    asyncHandler(async (req, res) => {
        ...
        if (req.user.userType === 'student' && req.user.userId !== id) {
            throw createError('PERMISSION_DENIED', '学生只能查看自己的排名信息');
        }
...
router.post('/', authenticateToken, requireTeacher,
    ...
    asyncHandler(async (req, res) => {
        ...
        if (!name || typeof name !== 'string' || name.trim().length === 0) {
            throw createError('VALIDATION_ERROR', '姓名不能为空');
        }
        
        if (!className || typeof className !== 'string' || className.trim().length === 0) {
            throw createError('VALIDATION_ERROR', '班级不能为空');
        }
        
        if (typeof balance !== 'number' || balance < 0) {
            throw createError('VALIDATION_ERROR', '初始积分必须为非负数');
        }
...
router.post('/batch', authenticateToken, requireTeacher,
    ...
    asyncHandler(async (req, res) => {
        ...
        if (!Array.isArray(students) || students.length === 0) {
            throw createError('VALIDATION_ERROR', '学生列表不能为空');
        }
        
        if (students.length > 100) {
            throw createError('VALIDATION_ERROR', '批量创建不能超过100个学生');
        }
...
router.put('/:id', authenticateToken, requireTeacher,
    ...
    asyncHandler(async (req, res) => {
        ...
        if (updateData.id && updateData.id !== id) {
            throw createError('VALIDATION_ERROR', '不能修改学号');
        }
