...
class PointsService {
  ...
  async getStudentRankInfo(studentId) {
    try {
      ...
      const totalRank =
        totalRanking.findIndex((item) => item.studentId === studentId) + 1;
      const dailyRank =
        dailyRanking.findIndex((item) => item.studentId === studentId) + 1;
      const weeklyRank =
        weeklyRanking.findIndex((item) => item.studentId === studentId) + 1;
